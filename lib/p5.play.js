...
(function (root, factory) {
  ...
  p5.prototype.updateSprites = function (upd) {
    if (upd == false) spriteUpdate = false;
    if (upd == true) spriteUpdate = true;
  ...
  p5.prototype.keyWentDown = function (key) {
    ...
    if (typeof key == "string") keyCode = KEY[key.toUpperCase()];
    else keyCode = key;

    //if undefined start checking it
    if (keyStates[keyCode] == undefined) {
      if (keyIsDown(keyCode)) keyStates[keyCode] = KEY_IS_DOWN;
  ...
  p5.prototype.keyWentUp = function (key) {
    ...
    //if undefined start checking it
    if (keyStates[keyCode] === undefined) {
      if (keyIsDown(key)) keyStates[keyCode] = KEY_IS_DOWN;
      else keyStates[keyCode] = KEY_IS_UP;
    }
  ...
  p5.prototype.mouseDown = function (buttonCode) {
    if (buttonCode == undefined) buttonCode = LEFT;
    else buttonCode = buttonCode;
  ...
  p5.prototype.readPresses = function () {
    ...
    for (var btn in mouseStates) {
      if (mouseIsPressed && mouseButton == btn) {
        //if is down
  ...
  p5.prototype.useQuadTree = function (use) {
    if (quadTree != undefined) {
      if (use == undefined) return quadTree.active;
      else if (use) quadTree.active = true;
      else quadTree.active = false;
    } else return false;
  };
  ...
  */
  ...
  this.update = function () {
    if (!this.removed) {
      ...
      if (this.rotateToDirection) this.rotation = this.getDirection();
      ...
      if (animations[currentAnimation] != null) {
        ...
        if (
          ...
          this.width =
            animations[currentAnimation].getWidth() * abs(this.scale);
          this.height =
            animations[currentAnimation].getHeight() * abs(this.scale);
      ...
      if (this.mouseActive) {
        ...
        if (this.collider == null) this.setDefaultCollider();
  ...
  this.mouseUpdate = function () {
    ...
    else mousePosition = createVector(mouseX, mouseY);
    ...
    if (this.collider != null) {
      ...
      //global p5 var
      if (this.mouseIsOver && mouseIsPressed) this.mouseIsPressed = true;
  ...
  this.setCollider = function (type, offsetX, offsetY, width, height) {
    ...
    if (type == "rectangle" && arguments.length == 5)
      this.collider = new AABB(
        this.position,
        createVector(arguments[3], arguments[4]),
        createVector(arguments[1], arguments[2])
      );
    else if (type == "circle") {
      ...
      if (this != others[i] && !this.removed) {
        ...
        if (this.collider != undefined && other.collider != undefined) {
          if (type == "overlap") {
            ...
            if (displacement.x == 0 && displacement.y == 0) result = false;
            else {
              if (!this.immovable) {
...
function cameraPush() {
  ...
  if (!camera.active) {
    ...
    scale(camera.zoom);
    translate(
      -camera.position.x + width / 2 / camera.zoom,
      -camera.position.y + height / 2 / camera.zoom
    );
  }
}
...
function Group() {
  ...
  array.minDepth = function () {
    ...
    if (array.length == 0) min = 99999;
    else min = array[0].depth;

    for (var i = 0; i < array.length; i++)
      if (array[i].depth < min) min = array[i].depth;
...
function CircleCollider(_center, _radius, _offset) {
  ...
  this.overlap = function (other) {
    ...
    r *= r;
    var sqDist =
      pow(this.center.x - other.center.x, 2) +
      pow(this.center.y - other.center.y, 2);
    return r > sqDist;
  };
  ...
  this.collide = function (other) {
    if (this.overlap(other)) {
      ...
      var intersection = abs(
        radii -
          dist(this.center.x, this.center.y, other.center.x, other.center.y)
      );
  ...
  this.collide = function (other) {
    ...
    //box vs circle
      ...
      if (distance < other.radius) {
        ...
        var displacement = createVector(
          cos(a) * other.radius - d.x,
          sin(a) * other.radius - d.y
        );
...
function Animation() {
  ...
  // Sprite sheet mode
    ...
    for (var i = 0; i < arguments.length; i++) {
      ...
      if (arguments[i] instanceof p5.Image) this.images.push(arguments[i]);
      else this.images.push(loadImage(arguments[i]));
    }
  }
  ...
  this.goToFrame = function (f) {
    ...
    if (this.f >= 0 && this.f < this.images.length) targetFrame = this.f;

    if (targetFrame != frame) this.playing = true;
  };
...
Quadtree.prototype.getIndex = function (pRect) {
  if (pRect.collider == null) return -1;
  else {
    var index = -1,
